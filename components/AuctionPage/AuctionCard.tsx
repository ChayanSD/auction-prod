...
/**
 * Auction Card Component for Auctions Listing Page
  ...
  return (
    <div className="group bg-white rounded-[20px] border border-[#E3E3E3] hover:shadow-lg transition-all duration-300 overflow-hidden flex flex-col h-full">
      ...
      <div className="p-4 sm:p-5 lg:p-6 flex-1 flex flex-col">
        ...
        {/* Auction Name */}
        <Link href={`/auction?auctionId=${id}`}>
          <h3 className="font-bold text-lg sm:text-xl lg:text-2xl text-[#0E0E0E] mb-2 sm:mb-3 hover:text-purple-600 transition-colors line-clamp-2">
            {name}
          </h3>
        </Link>
        ...
        {/* Description */}
        {description && (
          <p className="text-sm sm:text-base text-[#4D4D4D] mb-3 sm:mb-4 line-clamp-2 flex-1">
            {description}
          </p>
        )}

        {/* Location */}
        <div className="flex items-center gap-2 mb-3 sm:mb-4 text-sm sm:text-base text-[#4D4D4D]">
        ...
        {/* Item Count */}
        <div className="text-sm sm:text-base text-[#4D4D4D] mb-4 sm:mb-5">
          <span className="font-semibold text-[#0E0E0E]">{itemCount}</span>
          <span className="ml-1">{itemCount === 1 ? 'Item' : 'Items'}</span>
        </div>

        {/* Tags */}
        {tags && tags.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-4 sm:mb-5">
            {tags.slice(0, 3).map((tagOnAuction) => (
              <span
                key={tagOnAuction.tag.id}
                className="inline-block bg-[#F7F7F7] text-[#0E0E0E] text-xs font-medium px-3 py-1 rounded-full"
              >
                {tagOnAuction.tag.name}
              </span>
            ))}
            {tags.length > 3 && (
              <span className="inline-block bg-[#F7F7F7] text-[#0E0E0E] text-xs font-medium px-3 py-1 rounded-full">
                +{tags.length - 3}
              </span>
            )}
          </div>
        ...
        {/* View Items Button */}
        <Link href={`/auction?auctionId=${id}`} className="mt-auto">
          <button className="w-full text-center py-2.5 sm:py-3 px-4 sm:px-5 bg-gradient-to-bl from-[#9F13FB] to-[#E95AFF] text-white text-sm sm:text-base rounded-full hover:shadow-md transition-all active:scale-95 cursor-pointer font-semibold">
            View Items
          </button>
        </Link>
      </div>
